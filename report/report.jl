...
md"""
## Σκοπός της εργασίας
Στο τρίτο και τελευταίο μέρος των εργασίων του μαθήματος, αναλαμβάνουμε τη σκίαση και τη θέαση ενός τρισδιάστατου αντικειμένου χρησιμοποιώντας δύο διαφορετικούς αλγορίθμους, με τρία διαφορετικά είδη φωτισμού.
\
...
\
Γι' αυτό το σκοπό, για κάθε τρίγωνο, βρίσκουμε το normal vector του και το προσθέτουμε στα αντίστοιχα index των κορυφών του στον πίνακα *normals*, όπου αποθηκευέται το αποτέλεσμα που θα επιστραφεί. Αν $A, B, C$ είναι οι κορυφές ενός τυχαίου τριγώνου, τότε για το normal vector του:
